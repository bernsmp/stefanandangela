...
export default function StefanPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const res = await fetch('/api/data/stefan');
      ...
      <footer className="bg-[var(--grey-900)] text-white py-12 px-6 lg:px-8">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-[var(--grey-400)] text-sm">
            Stefan's Cognitive Fingerprint™ • Analysis by Max Bernstein
          </p>
        </div>
      </footer>
    </div>
